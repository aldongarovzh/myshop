...
*	Иницилазация сессионной переменной ($_SESSION['user'])
*
*	return json массив данных нового пользователя
*/
function registerAction($db){
    ...
    $resData = null;
    $resData = checkRegisterParams($db, $email, $pwd1, $pwd2);

    if(! $resData && checkUserEmail($db, $email)){ //если нет даннхы и возвращает тру
    ...
    if(! $resData){
        ...
        $pwdMD5 = md5($pwd1);
        $userData = registerNewUser($db, $email, $pwdMD5, $name, $phone, $adress);
...
function updateAction($db){
    ...
    //Обновление данных пользователя
    $res = updateUserData($db, $name, $phone, $adress, $pwd1, $pwd2, $curPwdMD5);
    if($res){
        $resData['success'] = 1;
