...
function removefromcartAction(){
	...
	if($key !== false){
		unset($_SESSION['cart'][$key]); //$key это ключ переменной $_SESSION['cart']
...
function orderAction($db, $smarty){
	...
	// Добавялем каждому продукту дополнительное поле
	...
	$i = 0;
	foreach($rsProducts as &$item){ // & - амперсант значит, если мы меняем переменную $item, то данное значение будет меняться и в переменной $rsProducts
...
function saveorderAction($db){
	...
	$orderId = makeNewOrder($db, $name, $phone, $adress);
	...
	// сохраянем товары для созданного заказа
